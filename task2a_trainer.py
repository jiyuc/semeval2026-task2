...
class DataPreprocessor:
    ...
    def _pair_sequences_by_user_id(self, dataset: Dataset) -> Dataset:
        ...
        for user_id, group in user_groups:
            ...
            # # pre-fill the first example in the group
            # user_pairs.append({
            #     'user_id': user_id,
            #     'text1': "[FIRST]",
            #     'text2': group.iloc[0]['text'],
            #     'state1': 0,  # neutral valence by default
            #     'time_diff': 0,
            #     f'{self.label}': 0,
            #     'timestamp1': group.iloc[0]['timestamp'],
            #     'timestamp2': group.iloc[0]['timestamp'],
            # })
            ...
            # # Handle the last sequence (no pair to compare with)
            # if len(group) > 1:
            #     last_example = group.iloc[-1]
            #     # Set a default value (0 or another placeholder) for the state_change of the last sequence
            #     # Set label to None or 0 for the last sequence where there is no state_change
            #     state_change_value = 0 if pd.isna(last_example[self.label]) else last_example[self.label]
            #     user_pairs.append({
            #         'user_id': user_id,
            #         'text1': last_example['text'],
            #         'text2': "[LAST]",
            #         'state1': last_example[f'{self.feature}'],
            #         'time_diff': 0,  # No time difference for the last sequence
            #         f'{self.label}': state_change_value,  # Set state_change to 0 if None
            #         'timestamp1': last_example['timestamp'],
            #         'timestamp2': last_example['timestamp'],
            #     })
...
class MetricComputer:
    ...
    def compute(self, eval_pred):
        ...
        labels = np.squeeze(labels).astype(np.float32)
