...
def load_and_preprocess(input_csv, tokenizer, label_col, history_N, max_length):
...
def main():
    ...
    for suffix in ["valence", "arousal"]:
        ...
        encoded_ds, template = load_and_preprocess(
            args.input_csv, tokenizer, f'disposition_change_{suffix}', args.at_N, args.max_length
        )
        ...
        meta_df[f"pred_dispo_change_{suffix}"] = [round(float(p), 2) for p in preds]

    meta_df = meta_df[["user_id","pred_dispo_change_valence","pred_dispo_change_arousal"]].copy()
