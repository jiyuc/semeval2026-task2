...
class DataPreprocessor:
    ...
    def pad_features(self, values, length=5):
        """
    ...
    def _generate_collection_phase_sample_by_user_id(self, dataset: Dataset) -> Dataset:
        ...
        for user_id, group in user_groups:
            ...
            for phase_id, collection in collection_phases:
                ...
                curr = collection.iloc[-2]  # collect the second last post
                time_diffs = []
                for i in range(1, len(collection) - 1):  # Exclude the time_diff between curr and future
                    slow = collection.iloc[i - 1]
                    fast = collection.iloc[i]
                    time_diffs.append(self._calculate_time_diff(fast, slow))

                # if len(time_diffs) == 0:
                #     time_diffs.append(self._calculate_time_diff(collection.iloc[1], collection.iloc[0]))
                ...
                phase_samples.append({
                    ...
                    'curr_valence': curr['valence'],
                    'curr_arousal': curr['arousal'],
                    'valences': self.pad_features(collection['valence'][:-2][::-1]),
                    'arousals': self.pad_features(collection['arousal'][:-2][::-1]),
                    'state_change_valences': self.pad_features(collection['state_change_valence'][:-2][::-1]),
                    'state_change_arousals': self.pad_features(collection['state_change_arousal'][:-2][::-1]),
                    'time_diffs': self.pad_features(time_diffs[::-1]),
                    f'{self.label}': curr[self.label]  # the state change in the last of each phase to predict
                })
    ...
    def combine_text(example):
        ...
        """
        return {
            'text': f"current state: {example['curr_valence']} {example['curr_arousal']} "
                    f"</s> text: {str(example['curr_text'])}"
        }
...
class RobertaRegressionHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.dense = nn.Linear(config.hidden_size + 27, config.hidden_size)
        ...
        self.fnn = nn.Sequential(
            ...
            )  # define a fnn for learning history data
    ...
    def forward(self, features, **kwargs):
        ...
        valences = kwargs.get("valences").view(B, 5)
        arousals = kwargs.get("arousals").view(B, 5)
        state_change_valences = kwargs.get("state_change_valences").view(B, 5)
        state_change_arousals = kwargs.get("state_change_arousals").view(B, 5)
        time_diffs = kwargs.get("time_diffs").view(B, 5)

        y = self.fnn(torch.cat([curr_valence,
                                curr_arousal,
                                valences,
                                arousals,
                                state_change_valences,
                                state_change_arousals,
                                time_diffs], dim=-1)) # use a layer to train relation
...
class CustomDataCollator:
    ...
    def __call__(self, features):
        ...
        for key in ["curr_valence", "curr_arousal", "valences", "arousals", "state_change_valences", "state_change_arousals", "time_diffs"]:
        ...
        #     'curr_valence': curr['valence'],
        #     'curr_arousal': curr['arousal'],
        #     'valences': self.pad_features(collection['valence'][:-1][::-1]),
        #     'arousals': self.pad_features(collection['arousal'][:-1][::-1]),
        #     'state_change_valences': self.pad_features(collection['state_change_valences'][:-2][::-1]),
        #     'state_change_arousals': self.pad_features(collection['state_change_arousals'][:-2][::-1]),
        #     'time_diffs': self.pad_features(time_diffs),
        #     f'{self.label}': curr[self.label]  # the state change in the last of each phase to predict
        # })
        return batch
